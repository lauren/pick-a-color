...
;(function ($) {
    ...
    $.fn.pickAColor = function (options) {
      ...
      var markupAfterInput = function () {
        ...
        if (useTabs) {
          var $tabContainer = $("<div>").addClass("color-menu-tabs"),
              ...
              recommendedColorsClass = settings.showBasicColors ? "recommendedColors-tab tab" : "recommendedColors-tab tab tab-active";
          ...
          if (settings.showRecommendedColors) {
            $tabContainer.append($("<span>").addClass(recommendedColorsClass).
            append($("<a>").text(settings.recommendedLabel)));
          }
		...
		if (settings.showRecommendedColors) {
          var recomendedColorsActiveClass = settings.showBasicColors ? 'inactive-content' : 'active-content',
            $recomendedColors = $("<div>").addClass("recommendedColors-content").addClass(recomendedColorsActiveClass);
          $dropdownContainer.append($recomendedColors);
        }
      ...
      var methods = {
		...
		updateRecommendedColorPreview: function (elements) {
    ...
    updateSavedColorPreview: function (elements) {
      $.each(elements, function (index) {
		...
		updateRecommendedColorMarkup: function ($recommendedColorsContent,myRecommendedColors) {
          if (settings.showSavedColors && myRecommendedColors.length > 0) {

            var maxRecommendedColors = myStyleVars.rowsInDropdown * myStyleVars.maxColsInDropdown;
            myRecommendedColors = myRecommendedColors.slice(0, maxRecommendedColors);

            var $col0 = $("<ul>").addClass("recommended-color-col 0"),
                $col1 = $("<ul>").addClass("recommended-color-col 1");

            $.each(myRecommendedColors, function (index,value) {
            ...
            $recommendedColorsContent.html($col0);
            $recommendedColorsContent.append($col1);

            var recommendedColorLinks = $($recommendedColorsContent).find("a");
            methods.updateRecommendedColorPreview(recommendedColorLinks);
      ...
      return this.each(function (index) {
        ...
        if (settings.showRecommendedColors) {
			myElements.recommendedColorsContent = myElements.thisWrapper.find(".recommendedColors-content");
		}
		...
		if (settings.showRecommendedColors) {
          ...
          // make the links in saved colors work
          $(myElements.recommendedColorsContent).click( function (event) {
            var $thisEl = $(event.target);
		  ...
		  if (settings.recommendedColors.length >0) {
			methods.updateRecommendedColorMarkup(myElements.recommendedColorsContent, settings.recommendedColors);
		  }
        }
